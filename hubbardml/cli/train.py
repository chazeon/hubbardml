...
class Trainer:
    ...
    def make_model(self) -> Tuple[nn.Module, type]:
        ...
        if missing:
            raise ValueError(f"Config species {missing} not found in data {data_elements}")
    ...
    def make_datasets(self, dataset_class: type, graph) -> Tuple[DataLoader, DataLoader]:
    ...
    def train_loop(self, model: nn.Module, train_loader: DataLoader, val_loader: DataLoader) -> Tuple[nn.Module, Dict[str, List[float]], float]:
        """Main training loop."""
    ...
    def train(self) -> Tuple[nn.Module, Dict[str, List[float]]]:
        ...
        # Run training
        model, history, _best_val_loss = self.train_loop(model, train_loader, val_loader)
...
class VDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        ...
        # Combine tensors
        site0_tensor = torch.hstack(tuple(site0_inputs[name] for name in self.site0_keys))
        site1_tensor = torch.hstack(tuple(site1_inputs[name] for name in self.site1_keys))
        
        # Distance between sites
        edge_tensor = torch.tensor([self.data.edge.dist[idx]], dtype=DTYPE, device=self.device)
        
        return {"site0": site0_tensor, "site1": site1_tensor, "edge": edge_tensor}, target
